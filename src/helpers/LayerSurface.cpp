...
CLayerSurface::CLayerSurface(SMonitor* pMonitor) : m_pMonitor(pMonitor) {
    pSurface = makeShared<CCWlSurface>(g_pHyprloupe->m_pCompositor->sendCreateSurface());

    if (!pSurface) {
        Debug::log(CRIT, "The compositor did not allow hyprloupe a surface!");
        g_pHyprloupe->finish(1);
        return;
    }

    if (!g_pHyprloupe->m_bNoFractional) {
        pViewport = makeShared<CCWpViewport>(g_pHyprloupe->m_pViewporter->sendGetViewport(pSurface->resource()));
        ...
        pFractionalScale = makeShared<CCWpFractionalScaleV1>(g_pHyprloupe->m_pFractionalMgr->sendGetFractionalScale(pSurface->resource()));
        pFractionalScale->setPreferredScale([this](CCWpFractionalScaleV1* r, uint32_t scale120) { //
            ...
            wantsReload     = true;
            g_pHyprloupe->recheckACK();
        });
    }

    pLayerSurface = makeShared<CCZwlrLayerSurfaceV1>(
        g_pHyprloupe->m_pLayerShell->sendGetLayerSurface(pSurface->resource(), pMonitor->output->resource(), ZWLR_LAYER_SHELL_V1_LAYER_OVERLAY, "hyprloupe"));

    if (!pLayerSurface) {
        Debug::log(CRIT, "The compositor did not allow hyprloupe a layersurface!");
        g_pHyprloupe->finish(1);
        return;
    }
    ...
    wl_display_flush(g_pHyprloupe->m_pWLDisplay);
}
...
static void onCallbackDone(CLayerSurface* surf, uint32_t when) {
    ...
    g_pHyprloupe->renderSurface(surf);
}
