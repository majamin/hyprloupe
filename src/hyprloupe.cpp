...
static void sigHandler(int sig) {
    g_pHyprloupe->m_bRunning.store(false);
}

void CHyprloupe::init() {
    ...
    m_pRegistry->setGlobal([this](CCWlRegistry* r, uint32_t name, const char* interface, uint32_t version) {
        if (strcmp(interface, wl_compositor_interface.name) == 0) {
            ...
            const auto PMONITOR = g_pHyprloupe->m_vMonitors
            ...
            m_pSeat->setCapabilities([this](CCWlSeat* seat, uint32_t caps) {
                if (caps & WL_SEAT_CAPABILITY_POINTER) {
                    ...
                    Debug::log(CRIT, "Hyprloupe cannot work without a pointer!");
                    g_pHyprloupe->finish(1);
                }
...
void CHyprloupe::finish(int code) {
    ...
    for (auto& ls : m_vLayerSurfaces) {
        if ((ls->wantsACK || ls->wantsReload) && ls->screenBuffer) {
            ...
            const auto MONITORSIZE =
                (ls->screenBuffer && !g_pHyprloupe->m_bNoFractional ? ls->m_pMonitor->size * ls->fractionalScale : ls->m_pMonitor->size * ls->m_pMonitor->scale).round();
...
SP<SPoolBuffer> CHyprloupe::getBufferForLS(CLayerSurface* pLS) {
...
bool CHyprloupe::setCloexec(const int& FD) {
...
int CHyprloupe::createPoolFile(size_t size, std::string& name) {
    ...
    name = std::string(XDGRUNTIMEDIR) + "/.hyprloupe_XXXXXX";
...
void CHyprloupe::convertBuffer(SP<SPoolBuffer> pBuffer) {
...
void* CHyprloupe::convert24To32Buffer(SP<SPoolBuffer> pBuffer) {
...
void CHyprloupe::renderSurface(CLayerSurface* pSurface, bool forceInactive) {
    ...
    if (pSurface == m_pLastSurface && !forceInactive && m_bCoordsInitialized) {
        ...
        if (m_fDimAlpha > 0.0f) {
            cairo_set_source_rgba(PCAIRO, 0, 0, 0, m_fDimAlpha);
            cairo_paint(PCAIRO);
        }
...
CColor CHyprloupe::getColorFromPixel(CLayerSurface* pLS, Vector2D pix) {
    pix = pix.floor();
